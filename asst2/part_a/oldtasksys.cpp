...
TaskSystemParallelThreadPoolSpinning::TaskSystemParallelThreadPoolSpinning(int num_threads): ITaskSystem(num_threads) {
    ...
    for(int j = 0; j < num_threads; j++){
        thread_handle.push_back(std::thread(dynamic_allocation_run_for_spin, std::ref(fake_runnable), std::ref(fake_num_total_tasks), std::ref(current_task_iteration), std::ref(my_lock), std::ref(kill_threads)));
...
void TaskSystemParallelThreadPoolSpinning::run(IRunnable* runnable, int num_total_tasks) {
    ...
    fake_runnable = runnable;//std::ref(runnable);
