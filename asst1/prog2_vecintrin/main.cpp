...
//  __cs149_vec_float zero = _cs149_vset_float(0.f);
  ...
  __cs149_mask maskAll, maskExpOne, maskExpIsNotOne, maskActiveWhileSet, maskLargeVal;
...
//  Why is that the case?
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    //maskAll = _cs149_init_ones();
    if((i + extraSet) == N){
	maskAll = _cs149_init_ones(extraSet);
    }
    // All zeros
    maskActiveWhileSet = _cs149_init_ones(0);
    maskExpOne = _cs149_init_ones(0);
    // Load vector of values from contiguous memory addresses
    _cs149_vload_float(x, values+i, maskAll);               // x = values[i];
    ...
    // Set mask according to predicate
    _cs149_veq_int(maskExpOne, y, zero, maskAll);     // if (y ==  0) {

    // Execute instruction using mask ("if" clause)
    _cs149_vset_float(result, 1.f , maskExpOne);
    _cs149_vstore_float(output+i, result, maskExpOne);


    // Inverse maskIsNegative to generate "else" mask
    maskExpIsNotOne = _cs149_mask_not(maskExpOne);     // } else {
    maskExpIsNotOne = _cs149_mask_and(maskExpIsNotOne, maskAll);
    // Execute instruction ("else" clause)
    _cs149_vmove_float(result, x, maskExpIsNotOne); 

    __cs149_vec_int count;

    _cs149_vsub_int(count, y, ones, maskExpIsNotOne);


    _cs149_vgt_int(maskActiveWhileSet, count, zero, maskExpIsNotOne);

    while(_cs149_cntbits( maskActiveWhileSet) > 0){
	_cs149_vmult_float(result, x, result, maskActiveWhileSet);
	_cs149_vsub_int(count, count, ones, maskActiveWhileSet);
        _cs149_vgt_int(maskActiveWhileSet, count, zero, maskActiveWhileSet);
    }

    //if result > 9.99999f
    maskLargeVal = _cs149_init_ones();
    maskLargeVal = _cs149_mask_and(maskLargeVal, maskExpIsNotOne);
    _cs149_vgt_float(maskLargeVal, result, nines, maskLargeVal);
   
    _cs149_vset_float(result, 9.999999f, maskLargeVal);


    // Write results back to memory
    _cs149_vstore_float(output+i, result, maskExpIsNotOne);
